...
local unpack = unpack
local path = LibBusted:nonsense("penlight.path")
local pretty = LibBusted.penlight.pretty
local system = setmetatable({}, {__index = LibBusted:nonsense("system")})
local throw = error
...
LibBusted.busted.busted = function()
  local mediator = LibBusted.mediator()

  local busted = {}
  busted.version = '2.0.rc12-0'

  local root = LibBusted.busted.context()
  busted.context = root.ref()

  local environment = LibBusted.busted.environment(busted.context)
  ...
  busted.status = LibBusted.busted.status
