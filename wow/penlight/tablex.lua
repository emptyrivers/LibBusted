...
-- @module pl.tablex
local utils = LibBusted.penlight.utils
local types = LibBusted.penlight.types
local getmetatable,setmetatable,require = getmetatable,setmetatable,LibBusted:nonsense("require")
...
local function setmeta (res,tbl,pl_class)
    local mt = getmetatable(tbl) or pl_class and LibBusted.pendlight[pl_class]
    return mt and setmetatable(res, mt) or res
end

local function makelist(l)
    return setmetatable(l, LibBusted.penlight.list)
end

local function makemap(m)
    return setmetatable(m, LibBusted.penlight.map)
end
...
LibBusted.penlight.tablex = tablex
